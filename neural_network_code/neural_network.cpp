...
void NeuralNetwork::propogateForward(RowVector& input)
{
	...
	// Propogate the data forward
	for (uint i = 1; i < topology.size(); i++) {
		(*neuronLayers[i]) = (*neuronLayers[i-1]) * (*weights[i-1]);
	}

	// Apply the activation function to the network
...
{
	// Calculate the errors made by the last layer
	(*deltas.back()) = output - (*neuronLayers.back());

	// Calculate the other errors differently, working our way from end to start
...
void NeuralNetwork::updateWeights()
{
	for(uint i = 0; i < topology.size() - 1, i++){
		if (i != topology.size() - 2){ // The last layer doesn't have any biases
